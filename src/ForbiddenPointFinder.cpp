...
auto C_EMPTY = Utils::EMPTY;
auto C_BLACK = Utils::BLACK;
auto C_WHITE = Utils::WHITE;
constexpr auto f_boardsize = Config::BOARD_SIZE;

CForbiddenPointFinder::CForbiddenPointFinder(const Utils::Board &board) {
...
void CForbiddenPointFinder::SetStone(int x, int y, Utils::STONE_COLOR cStone) { cBoard[x + 1][y + 1] = cStone; }
...
bool CForbiddenPointFinder::isForbidden(int x, int y) {
  ...
  x++;
  y++;
  if (cBoard[x][y] != C_EMPTY)
    return false;
  if (x >= 2 && x <= f_boardsize - 1 && y >= 2 && y <= f_boardsize - 1) {
    if (cBoard[x + 2][y + 2] == C_BLACK)
      nearbyBlack++;
    if (cBoard[x + 2][y] == C_BLACK)
      nearbyBlack++;
    if (cBoard[x + 2][y - 2] == C_BLACK)
      nearbyBlack++;
    if (cBoard[x][y + 2] == C_BLACK)
      nearbyBlack++;
    if (cBoard[x][y - 2] == C_BLACK)
      nearbyBlack++;
    if (cBoard[x - 2][y - 2] == C_BLACK)
    ...
    if (cBoard[x + 1][y - 1] == C_BLACK)
      nearbyBlack++;
    if (cBoard[x + 1][y] == C_BLACK)
      nearbyBlack++;
    if (cBoard[x + 1][y + 1] == C_BLACK)
      nearbyBlack++;
    if (cBoard[x][y - 1] == C_BLACK)
      nearbyBlack++;
    if (cBoard[x][y + 1] == C_BLACK)
      nearbyBlack++;
    if (cBoard[x - 1][y - 1] == C_BLACK)
