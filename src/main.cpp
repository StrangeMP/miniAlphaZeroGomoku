...
public:
  ...
  void show_game_setup() {
    ...
    //当前应该先手是白棋
    game.agent = std::make_unique<MCTS_Agent>(board, Utils::WHITE, game.net, game.node_table, config);
  ...
  void game_loop() {
    ...
    while (!game.game_ended && move_count < Config::BOARD_SQUARES) {
      Utils::Board &root_board = game.agent->get_root()->board_state;
      ...
      if (is_my_turn) {
        ...
        if(move_idx == 5){
          ...
          std::vector<int> n_moves = ai_get_n_moves(game.agent->get_root()->board_state, num_positions);
