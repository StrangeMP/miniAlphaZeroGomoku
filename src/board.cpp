...
const Direction DIRECTION[4] = {1, Board::MaxBoardSize - 1, Board::MaxBoardSize, Board::MaxBoardSize + 1};
...
bool Board::isOpenThree(Pos pos, Color piece, int iDir) {
  if (board[pos] != EMPTY)
    ...
    for (i = 1; i < 5; i++) {
      ...
      if (board[posi] == piece)
        continue;
      else if (board[posi] == EMPTY && isOpenFour(posi, piece, iDir) == OF_TRUE && !isDoubleFour(posi, piece) &&
...
ForbidChecker::ForbidChecker(const Utils::Board &board_) : board(Config::BOARD_SIZE, Config::BOARD_SIZE) {
  for (size_t r = 0; r < Config::BOARD_SIZE; ++r) {
    for (size_t c = 0; c < Config::BOARD_SIZE; ++c) {
      board.set(MakePos(r, c), stone2color(board_[r][c]));
    }
  }
}

bool ForbidChecker::isForbidden(Pos pos) { return board.isForbidden(pos) != FORBIDDEN_NONE; }