...
inline int check_threat_pattern(const Utils::Board &board, int r, int c, Utils::STONE_COLOR stone_color) {
  if (board[r][c] != Utils::EMPTY) {
    return 0; // 位置已经有棋子，无法形成威胁
  }

  // 临时放置一个棋子来测试威胁
  Utils::Board test_board = board;
    ...
    for (int k = 1; k < STONES_TO_WIN; ++k) {
      ...
      if (nr >= 0 && nr < B_SIZE && nc >= 0 && nc < B_SIZE) {
        if (test_board[nr][nc] == stone_color) {
          pos_count++;
          total_length++;
        } else if (test_board[nr][nc] == Utils::EMPTY) {
          open_ends++;
          break;
        } else {
          break; // 遇到对方棋子
...
// 检查棋盘上给定颜色的所有可能威胁
// 返回值: 包含所有威胁位置和类型的向量
inline std::vector<std::pair<Utils::Coord, int>> find_all_threats(const Utils::Board &board,
                                                                  Utils::STONE_COLOR stone_color) {
  std::vector<std::pair<Utils::Coord, int>> threats;
...
find_best_legal_action_for_threat_type(const std::vector<std::pair<Utils::Coord, int>> &threats, int target_threat_type,
                                       const std::array<bool, Config::BOARD_SQUARES> &legal_moves,
                                       const MCTS::MCTSAgent &mcts_agent, std::ostringstream &debug_stream,
  ...
  for (const auto &threat : threats) {
    if (threat.second == target_threat_type) {
      ...
      if (threat_idx >= 0 && threat_idx < Config::BOARD_SQUARES &&
          mcts_agent.root_node().children[threat_idx]) { // Assuming children is array-like and index is checked
        int visits = mcts_agent.root_node().children[threat_idx]->visit_count;
  ...
  if (best_action_candidate_idx != -1) {
    Utils::Coord coord = Utils::index_to_coordinate(best_action_candidate_idx);
    if (legal_moves[best_action_candidate_idx]) {
      ...
      debug_stream << log_success_prefix << " (" << coord.first << "," << coord.second << ")" << log_illegal_suffix_part
                   << "; ";
...
// 返回决定的动作索引和调试信息
inline std::pair<int, std::string> make_heuristic_decision(const Utils::Board &board_state,
                                                           Utils::STONE_COLOR agent_color,
                                                           Utils::STONE_COLOR enemy_color, int mcts_best_action_idx,
                                                           const std::array<bool, Config::BOARD_SQUARES> &legal_moves,
                                                           const MCTS::MCTSAgent &mcts_agent) {
  ...
  std::optional<int> p1_action = find_best_legal_action_for_threat_type(
      my_threats, 2, legal_moves, mcts_agent, debug_info, "[DEBUG] 选择我方冲五", "，但该点不合法");
  if (p1_action) {
  ...
  if (!heuristic_move_made) {
    std::optional<int> p2_action = find_best_legal_action_for_threat_type(
        enemy_threats, 2, legal_moves, mcts_agent, debug_info, "[DEBUG] 选择防守对方冲五", "，但该点不合法");
    if (p2_action) {
  ...
  if (!heuristic_move_made) {
    std::optional<int> p3_action = find_best_legal_action_for_threat_type(
        my_threats, 1, legal_moves, mcts_agent, debug_info, "[DEBUG] 选择我方活三进攻", "，但该点不合法");
    if (p3_action) {
  ...
  if (!heuristic_move_made) {
    std::optional<int> p4_action = find_best_legal_action_for_threat_type(
        enemy_threats, 1, legal_moves, mcts_agent, debug_info, "[DEBUG] 选择防守对方活三", "，但该点不合法");
    if (p4_action) {
