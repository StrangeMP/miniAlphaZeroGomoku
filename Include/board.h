...
enum Color { BLACK, WHITE, EMPTY, WALL, COLOR_NB = 2 };
...
constexpr int CoordX(Pos pos) { return (pos >> MAX_BOARD_SIZE_BIT) - BOARD_BOUNDARY; }

constexpr int CoordY(Pos pos) { return (pos & ((1 << MAX_BOARD_SIZE_BIT) - 1)) - BOARD_BOUNDARY; }
...
inline Color stone2color(Utils::STONE_COLOR stone) {
  switch (stone) {
    case Utils::BLACK:
      return BLACK;
    case Utils::WHITE:
...
class ForbidChecker {
public:
  ForbidChecker(const Utils::Board &board);
