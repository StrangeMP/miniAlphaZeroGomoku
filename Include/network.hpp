...
namespace Black {
    ...
    constexpr WEIGHT_T Q_ZERO = model_w_15_params::input_zero_point;
    constexpr WEIGHT_T Q_ONE = std::clamp(const_round_half_away_from_zero(1.0f/input_scale), WEIGHT_MIN, WEIGHT_MAX);
    constexpr WEIGHT_T Q_NEG_ONE = std::clamp(const_round_half_away_from_zero(-1.0f/input_scale), WEIGHT_MIN, WEIGHT_MAX);
...
inline QuantizedNetwork::QuantizedNetwork()
...
inline AlphaGomoku::GodNet::RetType QuantizedNetwork::feed(const Tensor<uint8_t, 3, 15, 15>& input) {
